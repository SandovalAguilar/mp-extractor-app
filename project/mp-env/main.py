...
tables = da.dataAnalyzer(hd.toDataFrame('https://www.misprofesores.com/escuelas/UANL-FCFM_2263'))
...
def main(page: ft.Page):
    datatable_max = ft.DataTable(columns = headers(tables.maxTable), rows = rows(tables.maxTable))
    datatable_min = ft.DataTable(columns = headers(tables.minTable), rows = rows(tables.minTable))
    datatable_describe = ft.DataTable(columns = headers(tables.describeData), rows = rows(tables.describeData))
    datatable_full = ft.DataTable(columns = headers(tables.fullTable), rows = rows(tables.fullTable))
